...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create ama-dcac
  fly apps create ama-dcac-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ama-dcac
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ama-dcac-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'ama-dcac-staging' does not match app name in config file 'ama-dcac'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name ama-dcac-db
  fly postgres attach --postgres-app ama-dcac-db --app ama-dcac

  fly postgres create --name ama-dcac-staging-db
  fly postgres attach --postgres-app ama-dcac-staging-db --app ama-dcac-staging
  ```
